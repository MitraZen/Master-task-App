...
-- Create function to automatically re-evaluate status based on dates
CREATE OR REPLACE FUNCTION check_overdue_tasks()
RETURNS TRIGGER AS $$
BEGIN
  -- If task is marked as done, set status to Complete (highest priority)
  IF NEW.done = TRUE THEN
    NEW.status = 'Complete';
    RETURN NEW;
  END IF;
  
  -- Don't change status if task is already complete
  IF NEW.status = 'Complete' AND NEW.done = FALSE THEN
    -- If task is unmarked as done, we need to re-evaluate
    -- Continue with status evaluation below
  END IF;
  
  -- Re-evaluate status when start_date or due_date changes
  -- Check if dates have changed
  IF (OLD.start_date IS DISTINCT FROM NEW.start_date) OR (OLD.due_date IS DISTINCT FROM NEW.due_date) OR (OLD.done IS DISTINCT FROM NEW.done) THEN
    
    -- If due_date is in the past and task is not complete -> Overdue
    IF NEW.due_date < CURRENT_DATE THEN
      NEW.status = 'Overdue';
    -- If due_date is today or in the future
    ELSIF NEW.due_date >= CURRENT_DATE THEN
      -- If start_date is in the future -> Not Started
      IF NEW.start_date > CURRENT_DATE THEN
        NEW.status = 'Not Started';
      -- If start_date is today or in the past, and due_date is in the future -> In Progress
      ELSIF NEW.start_date <= CURRENT_DATE THEN
        -- If it was overdue and now due date is moved to future -> In Progress
        IF OLD.status = 'Overdue' OR OLD.status IS NULL THEN
          NEW.status = 'In Progress';
        -- If it was In Progress, keep it as In Progress
        ELSIF OLD.status = 'In Progress' THEN
          NEW.status = 'In Progress';
        -- If it was Not Started and start_date is now <= current date -> In Progress
        ELSIF OLD.status = 'Not Started' AND NEW.start_date <= CURRENT_DATE THEN
          NEW.status = 'In Progress';
        -- Otherwise, if status wasn't explicitly set, default to In Progress for active tasks
        ELSE
          NEW.status = 'In Progress';
        END IF;
      END IF;
    END IF;
  ELSE
    -- Dates haven't changed, just check for overdue on existing task
    IF NEW.due_date < CURRENT_DATE AND NEW.status != 'Complete' AND NEW.done = FALSE THEN
      NEW.status = 'Overdue';
    END IF;
  END IF;
